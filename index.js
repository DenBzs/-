...
// ── Popup HTML (tab layout for mobile) ───────────────────────────────────────
function createPopupHtml(presets) {
    ...
    return `
        <div id="pmm-popup">
            ...
            <!-- Tab buttons -->
            <div class="pmm-tabs">
                <button class="pmm-tab pmm-tab-active" id="pmm-tab-src" data-tab="src">📤 출발</button>
                <button class="pmm-tab" id="pmm-tab-dst" data-tab="dst">📥 도착 &amp; 위치</button>
            </div>

            <!-- Tab: Source -->
            <div class="pmm-tab-panel" id="pmm-panel-src">
                <select id="pmm-source-preset" class="pmm-sel">
                    <option value="">-- 출발 프리셋 선택 --</option>
                    ${opts}
                </select>
                ...
                <button class="pmm-next-btn" id="pmm-go-dst">다음: 도착 프리셋 선택 →</button>
            </div>

            <!-- Tab: Destination -->
            <div class="pmm-tab-panel pmm-hidden" id="pmm-panel-dst">
                <select id="pmm-target-preset" class="pmm-sel">
                    <option value="">-- 도착 프리셋 선택 --</option>
                    ${opts}
                </select>
                <div class="pmm-list-header">
...
async function openPopup() {
    try {
        ...
        // callGenericPopup expects an HTML string or DOM node.
        // We pass HTML string so the popup renders it into the real DOM first,
        // then we query from the live DOM to wire up events.
        const popupHtml = createPopupHtml(presets);

        // Helper: get container from live DOM (after popup renders)
        function C() { return document.getElementById('pmm-popup'); }

        function switchTab(tab) {
            const c = C(); if (!c) return;
            c.querySelector('#pmm-panel-src').classList.toggle('pmm-hidden', tab !== 'src');
            c.querySelector('#pmm-panel-dst').classList.toggle('pmm-hidden', tab !== 'dst');
            c.querySelector('#pmm-tab-src').classList.toggle('pmm-tab-active', tab === 'src');
            c.querySelector('#pmm-tab-dst').classList.toggle('pmm-tab-active', tab === 'dst');
        }

        // Wire up events after a short delay so popup DOM is ready
        setTimeout(() => {
            const c = C(); if (!c) return;

            c.querySelector('#pmm-tab-src')?.addEventListener('click', () => switchTab('src'));
            c.querySelector('#pmm-tab-dst')?.addEventListener('click', () => switchTab('dst'));
            c.querySelector('#pmm-go-dst')?.addEventListener('click', () => switchTab('dst'));

            c.querySelector('#pmm-sel-all')?.addEventListener('click', () => {
                c.querySelectorAll('.pmm-check').forEach(cb => {
                    cb.checked = true;
                    selectedSourceIndices.add(parseInt(cb.dataset.index));
                    cb.closest('.pmm-item')?.classList.add('pmm-item-checked');
                });
                updateButtons(c);
                updateSelectionInfo(c);
            });

            c.querySelector('#pmm-sel-none')?.addEventListener('click', () => {
                c.querySelectorAll('.pmm-check').forEach(cb => {
                    cb.checked = false;
            ...
            c.querySelector('#pmm-source-preset')?.addEventListener('change', e => {
            ...
            c.querySelector('#pmm-target-preset')?.addEventListener('change', e => {
            ...
            c.querySelector('#pmm-btn-copy')?.addEventListener('click', () => performOperation(c, false));
            c.querySelector('#pmm-btn-move')?.addEventListener('click', () => performOperation(c, true));
        }, 100);

        await callGenericPopup(popupHtml, POPUP_TYPE.TEXT, '', {
            okButton: '닫기',
            cancelButton: false,
            wide: true,
        });
