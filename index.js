...
function renderTGGroups() {
    const area = document.getElementById('ptm-tg-groups-area');
    if (!area) return;
    const presetName = getCurrentPreset();
    if (!presetName) {
        area.innerHTML = '<div class="ptm-ph">프리셋이 선택되지 않았습니다</div>';
        return;
    }
    const groups = getGroupsForPreset(presetName);
    if (!groups.length) {
        area.innerHTML = '<div class="ptm-ph">그룹이 없습니다. [+ 그룹 추가]를 클릭하세요.</div>';
        return;
    }
    area.innerHTML = groups.map((g, gi) => buildGroupCard(g, gi, presetName)).join('');
    wireTGGroupEvents(area);
}

function wireTGGroupEvents(area) {
    area.querySelectorAll('.ptm-tg-onoff').forEach(btn => {
    ...
    area.querySelectorAll('.ptm-tg-rename').forEach(btn => {
        btn.addEventListener('click', () => {
            const gi = parseInt(btn.dataset.gi);
            const pn = getCurrentPreset();
            const groups = getGroupsForPreset(pn);
    ...
    area.querySelectorAll('.ptm-tg-delete-group').forEach(btn => {
        btn.addEventListener('click', () => {
            const gi = parseInt(btn.dataset.gi);
            const pn = getCurrentPreset();
            const groups = getGroupsForPreset(pn);
    ...
    area.querySelectorAll('.ptm-tg-copy-group').forEach(btn => {
        btn.addEventListener('click', () => copyGroupToPreset(parseInt(btn.dataset.gi), false));
    });
    area.querySelectorAll('.ptm-tg-move-group').forEach(btn => {
        btn.addEventListener('click', () => copyGroupToPreset(parseInt(btn.dataset.gi), true));
    });
    area.querySelectorAll('.ptm-tg-behavior').forEach(sel => {
    ...
    area.querySelectorAll('.ptm-tg-delete-toggle').forEach(btn => {
        btn.addEventListener('click', () => {
            const gi = parseInt(btn.dataset.gi);
    ...
    area.querySelectorAll('.ptm-tg-add-toggle').forEach(btn => {
    ...
    const toggleRows = g.toggles.map((t, ti) => {
        ...
        return `
            <div class="ptm-toggle-row">
                <span class="ptm-toggle-state ${g.isOn ? 'ptm-state-on' : 'ptm-state-off'}">${g.isOn ? (isDirect?'켜짐':'꺼짐') : (isDirect?'꺼짐':'켜짐')}</span>
                <span class="ptm-toggle-name">${name}</span>
                <select class="ptm-behavior" data-gi="${gi}" data-ti="${ti}">
                    <option value="direct" ${isDirect?'selected':''}>동일</option>
                    <option value="invert" ${!isDirect?'selected':''}>반전</option>
                </select>
                <button class="ptm-tg-delete-toggle ptm-ibtn ptm-ibtn-danger" data-gi="${gi}" data-ti="${ti}">삭제</button>
            </div>`;
    }).join('');

    return `
        <div class="ptm-group-card">
            <div class="ptm-group-header">
                <button class="ptm-tg-onoff ${g.isOn?'ptm-on':'ptm-off'}" data-gi="${gi}">${g.isOn?'켜짐':'꺼짐'}</button>
                <span class="ptm-group-name">${g.name}</span>
                <div class="ptm-group-actions">
                    <button class="ptm-tg-copy-group ptm-ibtn" data-gi="${gi}">복사</button>
                    <button class="ptm-tg-move-group ptm-ibtn" data-gi="${gi}">이동</button>
                    <button class="ptm-tg-rename ptm-ibtn" data-gi="${gi}">이름 변경</button>
                    <button class="ptm-tg-delete-group ptm-ibtn ptm-ibtn-danger" data-gi="${gi}">삭제</button>
                </div>
            </div>
            <div class="ptm-toggle-list">
                ${toggleRows || '<div class="ptm-ph" style="font-size:11px;padding:6px">토글 없음</div>'}
            </div>
            <button class="ptm-tg-add-toggle ptm-sm ptm-sm-full" data-gi="${gi}">+ 토글 추가</button>
        </div>`;
}
...
function copyGroupToPreset(gi, removeFromSource) {
    ...
    overlay.className = 'ptm-modal-overlay';
    overlay.innerHTML = `
        <div class="ptm-modal">
            <div class="ptm-modal-header">
                <span>"${group.name}" ${removeFromSource?'이동':'복사'} — 대상 프리셋 선택</span>
                <button class="ptm-ibtn" id="ptm-pcopy-close">닫기</button>
            </div>
            <div class="ptm-modal-list">
                ${allPresetNames.map(n => `
                    <div class="ptm-modal-item ptm-preset-item" data-preset="${n}"><span>${n}</span></div>
                `).join('')}
            </div>
        </div>`;
    ...
    overlay.querySelector('#ptm-pcopy-close').addEventListener('click', close);
    overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
    overlay.querySelectorAll('.ptm-preset-item').forEach(item => {
    ...
    overlay.innerHTML = `
        <div class="ptm-modal">
            <div class="ptm-modal-header">
                ...
                <button class="ptm-ibtn" id="ptm-modal-close">닫기</button>
            </div>
            <div style="padding:8px 12px">
                <input type="text" id="ptm-modal-search" class="ptm-text-input" placeholder="검색...">
            </div>
            <div class="ptm-modal-list" id="ptm-modal-list">
                ${allPrompts.map(p => {
                    const exists = existingTargets.has(p.identifier);
                    return `
                        <label class="ptm-modal-item ${exists?'ptm-modal-item-exists':''}">
                            <input type="checkbox" class="ptm-chk" data-id="${p.identifier}" ${exists?'disabled checked':''}>
                            <span>${p.name || p.identifier}</span>
                            ${exists?'<span class="ptm-exists-badge">추가됨</span>':''}
                        </label>`;
                }).join('')}
            </div>
            <div class="ptm-modal-footer">
                <button class="ptm-btn ptm-btn-copy" id="ptm-modal-confirm">선택 항목 추가</button>
            </div>
        </div>`;
    document.body.appendChild(overlay);

    overlay.querySelector('#ptm-modal-search').addEventListener('input', e => {
        const q = e.target.value.toLowerCase();
        overlay.querySelectorAll('.ptm-modal-item').forEach(item => {
            item.style.display = item.textContent.toLowerCase().includes(q) ? '' : 'none';
        });
    });
    const close = () => overlay.remove();
    overlay.querySelector('#ptm-modal-close').addEventListener('click', close);
    overlay.addEventListener('click', e => { if (e.target === overlay) close(); });
    overlay.querySelector('#ptm-modal-confirm').addEventListener('click', () => {
        const selected = [...overlay.querySelectorAll('#ptm-modal-list input:checked:not(:disabled)')]
        ...
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                ...
                <div class="ptm-block">
                    ...
                    <div id="ptm-src-list" class="ptm-list">
                        <div class="ptm-ph">출발 프리셋을 선택하세요</div>
                    </div>
                </div>
                <div class="ptm-block">
                ...
                <div class="ptm-block">
                    ...
                    <div id="ptm-dst-list" class="ptm-list">
                        <div class="ptm-ph">도착 프리셋을 선택하세요</div>
                    </div>
                </div>
                <div class="ptm-block ptm-group-block">
                    <label class="ptm-group-toggle-row">
                        <input type="checkbox" id="ptm-make-group" style="width:16px;height:16px;accent-color:#7a6fff;flex-shrink:0;cursor:pointer;margin:0">
                        <span>복사/이동 후 토글 그룹으로 묶기</span>
                    </label>
                    <div id="ptm-group-name-row" class="ptm-hidden" style="margin-top:8px">
                        <input type="text" id="ptm-group-name" class="ptm-text-input" placeholder="그룹 이름 입력...">
                    </div>
                </div>
                <div id="ptm-info" class="ptm-info">항목과 위치를 선택하면 버튼이 활성화됩니다</div>
                <div class="ptm-btn-row">
...
function buildTGDrawer() {
    ...
    el.innerHTML = `
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                <div id="ptm-tg-groups-area">
                    <div class="ptm-ph">로딩 중...</div>
                </div>
...
function renderSrcList() {
    ...
    el.innerHTML = sourceOrderedPrompts.map((entry, i) => {
        ...
        return `
            <label class="ptm-item ${!entry.enabled?'ptm-off':''} ${isChecked?'ptm-checked':''}">
                <input type="checkbox" class="ptm-chk" data-i="${i}" ${isChecked?'checked':''}>
                <span class="ptm-num">#${i+1}</span>
                <span class="ptm-name">${marker}${name}</span>
            </label>`;
    }).join('');
    el.querySelectorAll('.ptm-chk').forEach(cb => {
        cb.addEventListener('change', e => {
            const i = parseInt(e.target.dataset.i);
            if (e.target.checked) { selectedSourceIndices.add(i); e.target.closest('.ptm-item').classList.add('ptm-checked'); }
            else { selectedSourceIndices.delete(i); e.target.closest('.ptm-item').classList.remove('ptm-checked'); }
            updateButtons();
        });
    });
}
...
function renderDstList() {
    ...
    if (!targetOrderedPrompts.length) {
        el.innerHTML = `<div class="ptm-slot ${insertPosition===0?'ptm-slot-on':''}" data-slot="0">+</div>`;
        el.querySelector('.ptm-slot').addEventListener('click', () => selectSlot(0));
        return;
    }
    let html = `<div class="ptm-slot ${insertPosition===0?'ptm-slot-on':''}" data-slot="0">+</div>`;
    targetOrderedPrompts.forEach((entry, i) => {
        const name   = entry.prompt.name || entry.identifier || 'Unnamed';
        const marker = entry.prompt.marker ? '[고정] ' : '';
        html += `
            <div class="ptm-dst-item ${!entry.enabled?'ptm-off':''}">
                <span class="ptm-num">#${i+1}</span>
                <span class="ptm-name">${marker}${name}</span>
            </div>
            <div class="ptm-slot ${insertPosition===i+1?'ptm-slot-on':''}" data-slot="${i+1}">+</div>`;
    });
    el.innerHTML = html;
    el.querySelectorAll('.ptm-slot').forEach(s => s.addEventListener('click', () => selectSlot(parseInt(s.dataset.slot))));
}
...
function updateButtons() {
    ...
    document.getElementById('ptm-copy').disabled = !canAct;
    document.getElementById('ptm-move').disabled = !(canAct && sourcePresetName !== targetPresetName);
    const info = document.getElementById('ptm-info');
    if (!info) return;
...
async function performOperation(removeFromSource) {
    ...
    const groupName = document.getElementById('ptm-group-name')?.value.trim();
    if (makeGroup && !groupName) {
        toastr.warning('그룹 이름을 입력해주세요');
        document.getElementById('ptm-group-name')?.focus();
        return;
    }
    ...
    try {
        ...
        document.getElementById('ptm-group-name-row')?.classList.add('ptm-hidden');
        const gni = document.getElementById('ptm-group-name'); if (gni) gni.value = '';
        renderSrcList(); renderDstList(); updateButtons();

    } catch (err) {
        console.error('[PTM]', err);
...
function wireMoverEvents() {
    ...
    document.getElementById('ptm-all')?.addEventListener('click', () => {
        document.querySelectorAll('#ptm-src-list .ptm-chk').forEach(cb => {
            ...
            cb.closest('.ptm-item').classList.add('ptm-checked');
        });
        updateButtons();
    });
    document.getElementById('ptm-none')?.addEventListener('click', () => {
        document.querySelectorAll('#ptm-src-list .ptm-chk').forEach(cb => {
            cb.checked = false; cb.closest('.ptm-item').classList.remove('ptm-checked');
        });
    ...
    document.getElementById('ptm-range')?.addEventListener('click', () => {
        ...
        document.querySelectorAll('#ptm-src-list .ptm-chk').forEach(cb => {
            ...
            if (i >= min && i <= max) { cb.checked = true; cb.closest('.ptm-item').classList.add('ptm-checked'); }
        });
        updateButtons();
    });
    document.getElementById('ptm-make-group')?.addEventListener('change', e => {
        const row = document.getElementById('ptm-group-name-row');
        e.target.checked ? row.classList.remove('ptm-hidden') : row.classList.add('ptm-hidden');
        if (e.target.checked) document.getElementById('ptm-group-name')?.focus();
    });
...
function mount() {
    ...
    target.before(tgDrawer);
    tgDrawer.before(moverDrawer);
...
jQuery(async () => {
    ...
    try {
        await initImports();
        let count = 0;
        const t = setInterval(() => { if (mount() || ++count > 50) clearInterval(t); }, 200);
