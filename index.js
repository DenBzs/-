...
// ══════════════════════════════════════════
// B. Apply group → update prompt_order in-place + ST re-render
// ══════════════════════════════════════════
...
function applyGroup(pn, gi) {
    ...
    // Re-render ST prompt list via the prompt manager
    try {
        const pm = setupChatCompletionPromptManager(oai_settings);
        pm.render();
        pm.saveServiceSettings();
    } catch (e) {
        console.warn('[PTM] pm render failed', e);
    }
}
...
function buildGroupCard(g, gi, pn) {
    ...
    const rows = g.toggles.map((t, ti) => {
        ...
        // effective state: what this toggle will be when group is On
        const onState  = isDirect ? 'On' : 'Off';
        const offState = isDirect ? 'Off' : 'On';
        return `
        <div class="ptm-trow">
            <span class="ptm-tstate ${g.isOn ? 'ptm-ts-on' : 'ptm-ts-off'}">${g.isOn ? onState : offState}</span>
            <span class="ptm-tname">${name}</span>
            <select class="ptm-bsel" data-gi="${gi}" data-ti="${ti}" style="width:40px!important;min-width:40px!important;max-width:40px!important;flex-shrink:0!important;padding:2px 0!important;font-size:11px!important">
...
function wireGroupCards(area) {
    ...
    area.querySelectorAll('.ptm-ren-grp').forEach(btn => btn.addEventListener('click', () => {
        ...
        const n = prompt('그룹 이름 변경:', gs[gi].name);
        if (!n?.trim()) return;
    ...
    area.querySelectorAll('.ptm-del-grp').forEach(btn => btn.addEventListener('click', () => {
        const gi = +btn.dataset.gi, pn = getCurrentPreset(), gs = getGroupsForPreset(pn);
        if (!confirm(`"${gs[gi].name}" 삭제?`)) return;
...
// ── Modal — appended to document.body, high z-index ────────────
...
function showPresetPicker(gi, isMove) {
    ...
    const modal = buildModal(
        ...
        `<div class="ptm-mlist">${others.map(n=>`<div class="ptm-mitem ptm-preset-pick" data-preset="${n}">${n}</div>`).join('')}</div>`
    );
    modal.querySelectorAll('.ptm-preset-pick').forEach(item => item.addEventListener('click', () => {
        const dst = item.dataset.preset, dg = getGroupsForPreset(dst);
        let name = g.name, c = 1;
...
function showAddToggleModal(gi) {
    ...
    const modal = buildModal(
        `토글 추가 — ${gs[gi].name}`,
        `<div style="padding:8px 12px"><input type="text" id="ptm-msearch" class="ptm-tinput" placeholder="검색..."></div>
        ...
        <div class="ptm-mfoot"><button class="ptm-btn ptm-btn-copy" id="ptm-mconfirm">선택 항목 추가</button></div>`
    );
    ...
    modal.querySelector('#ptm-mconfirm').addEventListener('click', () => {
        ...
        const gs2 = getGroupsForPreset(getCurrentPreset());
        sel.forEach(id => gs2[gi].toggles.push({ target:id, behavior:'direct' }));
        saveGroups(getCurrentPreset(), gs2); renderTGGroups(); modal.remove();
...
function wireTG() {
    document.getElementById('ptm-add-group')?.addEventListener('click',()=>{
        ...
        const name=prompt('새 그룹 이름:');if(!name?.trim())return;
